...
class ChangeCartNotification extends Notification
{
    ...
    {
        $this->cart->ar_title = sprintf("تم تغير حالة الطلب %s",$this->cart->id);
        $this->cart->ar_body = sprintf("تم تغير حالة الطلب %s الى %s",$this->cart->id,__($this->cart->status));
        $this->cart->en_title = sprintf("status form cart %s changed",$this->cart->id);
        $this->cart->en_body = sprintf("status for cat %s changed to %s",$this->cart->id,$this->cart->status);
